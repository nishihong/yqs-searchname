...
// 		$normalHtml .= '</select>';
		...
		$pageHtml = str_ireplace(
			...
			array('<span class="PAGER_TOTAL">' .$count . $this->_config['record'] . '</span>', $prevHtml, $nextHtml, $listHtml, $firstHtml, $lastHtml, ''),
			$this->_format
		);
		unset($count, $totalPages, $nowPage, $prevHtml, $nextHtml, $listHtml, $firstHtml, $lastHtml);
		return $pageHtml;
	}
	...
	private function _url($page){
		...
		if ($params === null){
			$params = extend(input('param.'), input('post.'));
		}
...
// 		dump($params_new);
// 		exit;
		return url(request()->controller().'/'.request()->action(), $params);
	}
}
