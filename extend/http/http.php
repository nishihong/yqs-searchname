...
class http{
    ...
    /**
     * curl获取，模拟浏览器get请求，无视重定向次数
     * @param $url
     * @return bool|string
     */
    public function get_follow_resource($url)
    {
        if($url == "" || $this->TimeOut <= 0){
            return false;
        }
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, $url);
        ...
        curl_setopt($ch, CURLOPT_HTTPHEADER, $this->header); //添加自定义的http header
        return curl_exec($ch);
    }
    ...
    * @return mixed
    +-----------------------------------------------------------
    */
    public static function fsockopen_download($url, $conf = array()) {
        $return = '';
     ...
     * @return string
     +----------------------------------------------------------
     */
    public static function get_header_info($header='',$echo=true)
    {
        ob_start();
