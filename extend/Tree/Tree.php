...
class Tree {
	...
	public function __construct($data = [], $pk = 'id' ,$pfield = 'pid' ,$pvalue = '0'){
		$this->_data	= $data;
		$this->_pk		= $pk;
		$this->_pfield	= $pfield;
		$this->_pvalue	= $pvalue;
	...
	public function get_tree($id, $grade=1, $setPrefix = true){
		...
		if ($total > 0) {
			...
			$idx = 1;
			$sufixStr = $this->repeat(self::$_icon[3], $grade, true, '').' ';
			$preStr = $this->repeat(self::$_icon[4], $grade, true, '');
    ...
    public function repeat($mark, $num, $string = false, $glue = ',') {
        $num		 = (int) $num;
        if ( $num <= 0 )
            return $string ? '' : [];
        for ($i = 0; $i < $num; $i++)
