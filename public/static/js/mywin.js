...
function mywin(container, tags, func){
	...
	var position = function(){
		var config = {position : "absolute",left : 0,background : "#000000",opacity : "0.3",zIndex : 1000000};
		...
		$("#"+overlay).css(config).css({top:0,left:0,width:"100%",height:$(window).height()});
		$("#"+mywin).css({position : "absolute",left:"50%",top:"50%",margin:"-"+(width+$("#"+mywin).height())/2+"px 0px 0px -"+(width+$("#"+mywin).width())/2+"px",zIndex : 1000001,background:"#fff"});
	...
	var wininit = function(_container){
		...
		$container.bind(eventName, function(){
			...
			var a = this.href ||this.rel || this.alt || this.action;
