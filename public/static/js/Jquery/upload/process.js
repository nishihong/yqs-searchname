	...
	if($("#"+cid+"-box .upload-completed").length > 0){
		...
		if($trues.length > 0){
			...
			if($("#"+cid+"-box").hasClass("can-multi-file")){
				values = $.trim($("#uploadify-"+name).val());
				_values = values.split(",");
			}else{
				$("#uploadify-container-"+name).html("");
			}
			$trues.each(function(){
				...
				if($.inArray(vid, _values)==-1 && vid>0 && $("#uploadify-item-"+vid).length<=0){
					...
					if($img.length>0){
						...
						$("#uploadify-container-"+name).append("<span class='uploadify-fileItms' id='uploadify-item-"+vid+"'>"+_html+"<span onclick='uploadify_remove("+vid+")' class='uploadify-cancle' title='移除'></span></span>");
					}else{
						...
						$("#uploadify-container-"+name).append("<span class='uploadify-fileItms' id='uploadify-item-"+vid+"' title='"+$hide.children(".fileName").text()+"'>"+_html+"<span onclick='uploadify_remove("+vid+")' class='uploadify-cancle' title='移除'></span></span>");
					}
					
					lightboxShow("#uploadify-item-"+vid+" .lightbox");
				}
			});
			$("#uploadify-"+name).attr("value",values);
			$('#MYWIN_BOX .mywin-title .close').trigger('click');
		}else{
