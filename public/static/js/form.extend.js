...
jQuery.fn.extend({
	...
	_getTitle : function(){
		...
		var title = $.trim($label.html()).replace(new RegExp(/[：: ]*/g),'');
		return title;
	},
	//组装url地址参数
	_getParams : function(){
				...
				success : function(dat){
					if(dat){
						if(dat.code == 1){
                            ...
                            $this.parents('.edit-item').removeClass('i_err');
						}else{
							success = false;
	...
	_msg : function(msg,_type,force){
        ...
        msg =  msg.replace(/&nbsp;/g, "");
		$this.parent().append("<span class='MSG_BOX "+_type+"'>"+msg+"</span>");
        if(_type != "success"){
		    $(this).parents('.edit-item').addClass('i_err');
        }
	},
...
jQuery.extend({
	...
	toAjaxForm : function(){
		if($(this._form).hasClass("toAjaxForm")){
			...
			var type = $(this._form).attr('enctype') ? $(this._form).attr('enctype') : "application/x-www-form-urlencoded";
			if($._submiting){
...
//		        processData: false,
				...
				success : function(dat){
					...
					$._submiting = false;
                    if(dat.refresh==2){
                        $.msgbox(dat.info, dat.status, document.referrer ,dat.wait);
                    }else if(dat.refresh==3){
                        $.msgbox(dat.info, dat.status, location.href ,dat.wait);
                    }else{
			...
			var html = '<div class="Alert_Background"></div>' +
						 '<div class="Alert_Content">' +
							'<div class="In_Content">' +
								'<div class="header header_t_' + code + '"><i class="iconfont">' + (this._icos[code]!=undefined?this._icos[code]:this._icos['3']) + '</i>信息提示</div>' +
								'<div class="container">' +
