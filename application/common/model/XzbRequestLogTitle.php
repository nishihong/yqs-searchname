...
class XzbRequestLogTitle extends Common
{
    /**
     * 获取某天标题的汇总数据
    ...
    public function getDayData($time_day)
    {
        // 获取所有标题列表
        $title_condition = [];
        $title_condition[] = ['addtime_day', '=', $time_day];
        $all_title = model('XzbRequestLog')->field('url,title')->where($title_condition)->group('url')->select();
